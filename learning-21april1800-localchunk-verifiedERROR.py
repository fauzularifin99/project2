...
cursor.execute(
    """
    CREATE TABLE IF NOT EXISTS sumber_belajar_chunk (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        sumber_id INTEGER,
        judul TEXT,
        chunk_no INTEGER,
...
def buat_chunk(text, panjang=3000):
    chunks = []
    posisi = 0

    while posisi < len(text):
        chunks.append(text[posisi:posisi + panjang])
        posisi += panjang

    return chunks


def simpan_chunk(sumber_id, judul, isi):
    cursor.execute("DELETE FROM sumber_belajar_chunk WHERE sumber_id=?", (sumber_id,))

    chunks = buat_chunk(isi)
    nomor = 1

    for chunk in chunks:
        cursor.execute(
            "INSERT INTO sumber_belajar_chunk (sumber_id, judul, chunk_no, isi_chunk, dibuat_pada) VALUES (?, ?, ?, ?, ?)",
            (
                sumber_id,
                judul,
                nomor,
...
with tab1:
    ...
    if st.button("Simpan Paste Text"):
        if judul_text == "" or isi_text == "":
            ...
            sumber_id = cursor.lastrowid
            simpan_chunk(sumber_id, judul_text, isi_text)
            conn.commit()
    ...
    if st.button("Simpan File Upload"):
        if judul_file == "" or file_upload is None:
            ...
            if file_upload.name.endswith(".pdf"):
                ...
                ringkasan = "File PDF disimpan penuh sebagai chunk. Ringkasan tidak dibuat agar isi PDF tidak terpotong."
            else:
                isi_file = file_upload.read().decode("utf-8", errors="ignore")
                ringkasan = ringkas_text(isi_file)
            if isi_file.strip() == "":
                st.error("Isi file tidak terbaca. Jika PDF berupa scan/gambar, perlu OCR.")
                st.stop()
            ...
            sumber_id = cursor.lastrowid
            simpan_chunk(sumber_id, judul_file, isi_file)
            conn.commit()
    ...
    if st.button("Ambil dan Simpan URL"):
        if judul_url == "" or input_url == "":
            ...
            if isi_url.startswith("ERROR_URL:"):
                ...
                sumber_id = cursor.lastrowid
                simpan_chunk(sumber_id, judul_url, isi_url)
                conn.commit()
            ...
            with st.expander(f"{id_data} | {jenis} | {judul} | {status}"):
                ...
                with col3:
                    if st.button("Hapus", key=f"hapus_{id_data}"):
                        cursor.execute("DELETE FROM sumber_belajar_chunk WHERE sumber_id=?", (id_data,))
