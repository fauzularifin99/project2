...
try:
    cursor.execute("ALTER TABLE sumber_belajar ADD COLUMN openai_file_id TEXT")
except:
    pass

try:
    cursor.execute("ALTER TABLE sumber_belajar ADD COLUMN vector_store_id TEXT")
except:
    pass
    ...
    if st.button("Simpan File Upload"):
        if judul_file == "" or file_upload is None:
            ...
            if file_upload.name.endswith(".pdf"):
                ...
                isi_file = ""
                ringkasan = "PDF disimpan di OpenAI Vector Store. Isi tidak diringkas manual."
            ...
            cursor.execute(
                "INSERT INTO sumber_belajar (jenis, judul, sumber, isi, ringkasan, catatan, status, dibuat_pada, openai_file_id, vector_store_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    "FILE",
                    judul_file,
                    file_upload.name,
...
with tab4:
    ...
    cursor.execute("SELECT id, jenis, judul, sumber, ringkasan, catatan, status, dibuat_pada, openai_file_id, vector_store_id FROM sumber_belajar ORDER BY id DESC")
    rows = cursor.fetchall()
    ...
    if len(rows) == 0:
        ...
        for row in rows:
            ...
            status = row[6]
            dibuat_pada = row[7]
            openai_file_id = row[8]
            vector_store_id = row[9]
