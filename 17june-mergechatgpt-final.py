...
conn_belajar = sqlite3.connect("belajar.db", check_same_thread=False)
cursor_belajar = conn_belajar.cursor()
...
def ambil_konteks_aktif():
    cursor_belajar.execute("SELECT judul, isi FROM sumber_belajar WHERE status='AKTIF' ORDER BY id DESC")
    rows = cursor_belajar.fetchall()
...
# =========================
    ...
    kolom_video, kolom_chat = st.columns([1.05, 1])
    ...
    with kolom_chat:
        ...
        st.markdown('<div class="chat-title">RIWAYAT CHAT</div>', unsafe_allow_html=True)
        ...
        for i in range(len(st.session_state.CHATGPT)):
            ...
            # BOT (kiri)
            st.markdown(
                f'<div class="bubble-bot"><span class="chat-icon">AI:</span>{html.escape(st.session_state.CHATGPT[i])}</div>',
                unsafe_allow_html=True
            )
            ...
            if i < len(st.session_state.USERRESPONSE):
                st.markdown(
                    f'<div class="bubble-user">{html.escape(st.session_state.USERRESPONSE[i])}</div>',
                    unsafe_allow_html=True
                )
    ...
    with st.form("form_chat", clear_on_submit=True):
        col_stop, col_input, col_send = st.columns([1.2, 5, 1.2])
