...
# =========================
    ...
    chat_box = st.container(height=500, border=True, autoscroll=True)
    ...
    with chat_box:
        ...
        for i in range(len(st.session_state.CHATGPT)):
            ...
            # BOT (kiri)
            st.markdown(
                f'<div class="bubble-bot">{st.session_state.CHATGPT[i]}</div>',
                unsafe_allow_html=True
            )
            ...
            if i < len(st.session_state.USERRESPONSE):
                st.markdown(
                    f'<div class="bubble-user">{st.session_state.USERRESPONSE[i]}</div>',
                    unsafe_allow_html=True
                )
