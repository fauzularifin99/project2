...
# =========================
    ...
    # ---- berubah mulai sini ----
    with st.form("form_chat", clear_on_submit=True):      # <-- berubah
        jawaban_user = st.text_area("Jawaban user")       # <-- berubah
        kirim = st.form_submit_button("Kirim")            # <-- berubah
    # ---- berubah selesai ----
    ...
    with col2:
        if st.button("Berhenti"):
            st.session_state.MESSAGES.append({
                "role": "user",
                "content": "Saya ingin berhenti tanya jawab. Tolong tampilkan ringkasan analisa akhir dengan format: Identitas Siswa, Jenis Temuan, Skor Risiko, Rekomendasi Tindakan."
            })
            response = client.responses.create(
                model="gpt-5.4-nano",
                input=st.session_state.MESSAGES
