...
if st.session_state.halaman == 1:
    ...
    if st.button("CHECK"):
        if NAMA == "" or TTL is None:
            ...
            cursor_belajar.execute(
                "SELECT NISN, KELAS, HP, EMAIL FROM SISWA WHERE UPPER(NAMA)=? AND TTL=?",
                (NAMA, TTL_TEXT)
            )
            row = cursor_belajar.fetchone()

            if row:
                st.session_state.NISN_CHECK = row[0]
                st.session_state.KELAS_CHECK = row[1]
                st.session_state.HP_CHECK = row[2]
                st.session_state.EMAIL_CHECK = row[3]
